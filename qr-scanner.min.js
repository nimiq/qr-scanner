...
this._legacyOnDecode=b);b="object"===typeof c?c:{};this._onDecodeError=b.onDecodeError||("function"===typeof c?c:this._onDecodeError);this._calculateScanRegion=b.calculateScanRegion||("function"===typeof d?d:this._calculateScanRegion);this._preferredCamera=b.preferredCamera||e||this._preferredCamera;this._legacyCanvasSize="number"===typeof c?c:"number"===typeof d?d:this._legacyCanvasSize;this._maxScansPerSecond=b.maxScansPerSecond||this._maxScansPerSecond;this._onPlay=this._onPlay.bind(this);this._onLoadedMetaData=
this._onLoadedMetaData.bind(this);this._onVisibilityChange=this._onVisibilityChange.bind(this);this._updateOverlay=this._updateOverlay.bind(this);a.disablePictureInPicture=!0;a.playsInline=!0;a.muted=!0;let h=!1;a.hidden&&(a.hidden=!1,h=!0);document.body.contains(a)||(document.body.appendChild(a),h=!0);c=a.parentElement;if(b.highlightScanRegion||b.highlightCodeOutline)d=!!b.overlay,this.$overlay=b.overlay||document.createElement("div"),e=this.$overlay.style,e.position="absolute",e.display="none",
e.pointerEvents="none",this.$overlay.classList.add("scan-region-highlight"),!d&&b.highlightScanRegion&&(e.outline="#e9b213 solid 5px",c.insertBefore(this.$overlay,this.$video.nextSibling)),b.highlightCodeOutline&&(this.$codeOutlineHighlight=document.createElementNS("http://www.w3.org/2000/svg","svg"),b=document.createElementNS("http://www.w3.org/2000/svg","polygon"),this.$codeOutlineHighlight.appendChild(b),this.$overlay.appendChild(this.$codeOutlineHighlight),b=this.$codeOutlineHighlight.style,b.width=
...
!1,a;}}async turnFlashOff(){this._flashOn&&(this._flashOn=!1,await this._restartVideoStream())}destroy(){this.$video.removeEventListener("loadedmetadata",this._onLoadedMetaData);this.$video.removeEventListener("play",this._onPlay);document.removeEventListener("visibilitychange",this._onVisibilityChange);window.removeEventListener("resize",this._updateOverlay);this._destroyed=!0;this._flashOn=!1;this.stop();f._postWorkerMessage(this._qrEnginePromise,"close")}async start(){if(!(this._active&&!this._paused||
this._destroyed||("https:"!==window.location.protocol&&console.warn("The camera stream is only accessible if the page is transferred via https."),this._active=!0,document.hidden)))if(this._paused=!1,this.$video.srcObject)await this.$video.play();else try{let {stream:a,facingMode:b}=await this._getCameraStream();!this._active||this._paused?f._stopVideoStream(a):(this._setVideoMirror(b),this.$video.srcObject=a,await this.$video.play(),this._flashOn&&(this._flashOn=!1,this.turnFlashOn().catch(()=>{})))}catch(a){if(!this._paused)throw this._active=
!1,a;}}stop(){this.pause();this._active=!1}async pause(a=!1){this._paused=!0;if(!this._active)return!0;this.$video.pause();this.$overlay&&(this.$overlay.style.display="none");let b=()=>{this.$video.srcObject instanceof MediaStream&&(f._stopVideoStream(this.$video.srcObject),this.$video.srcObject=null)};if(a)return b(),!0;await new Promise(c=>setTimeout(c,300));if(!this._paused)return!1;b();return!0}async setCamera(a){a!==this._preferredCamera&&(this._preferredCamera=a,await this._restartVideoStream())}static async scanImage(a,
b,c,d,e=!1,h=!1){let m,n=!1;b&&("scanRegion"in b||"qrEngine"in b||"canvas"in b||"disallowCanvasResizing"in b||"alsoTryWithoutScanRegion"in b||"returnDetailedScanResult"in b)?(m=b.scanRegion,c=b.qrEngine,d=b.canvas,e=b.disallowCanvasResizing||!1,h=b.alsoTryWithoutScanRegion||!1,n=!0):b||c||d||e||h?console.warn("You're using a deprecated api for scanImage which will be removed in the future."):console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true.");
b=!!c;try{let q,k;[c,q]=await Promise.all([c||f.createQrEngine(),f._loadImage(a)]);[d,k]=f._drawToCanvas(q,m,d,e);if(c instanceof Worker){let g=c;b||g.postMessage({type:"inversionMode",data:"both"});return await new Promise((l,p)=>{let u,v,t;v=r=>{"qrResult"===r.data.type&&(g.removeEventListener("message",v),g.removeEventListener("error",t),clearTimeout(u),null!==r.data.data?l(n?{data:r.data.data,cornerPoints:f._convertPoints(r.data.cornerPoints,m)}:r.data.data):p(f.NO_QR_CODE_FOUND))};t=r=>{g.removeEventListener("message",
v);g.removeEventListener("error",t);clearTimeout(u);p("Scanner error: "+(r?r.message||r:"Unknown Error"))};g.addEventListener("message",v);g.addEventListener("error",t);u=setTimeout(()=>t("timeout"),1E4);let w=k.getImageData(0,0,d.width,d.height);g.postMessage({type:"decode",data:w},[w.data.buffer])})}return await Promise.race([new Promise((g,l)=>window.setTimeout(()=>l("Scanner error: timeout"),1E4)),(async()=>{try{let [g]=await c.detect(d);if(!g)throw f.NO_QR_CODE_FOUND;return n?{data:g.rawValue,
cornerPoints:f._convertPoints(g.cornerPoints,m)}:g.rawValue}catch(g){throw`Scanner error: ${g.message||g}`;}})()])}catch(q){if(!m||!h)throw q;let k=await f.scanImage(a,{qrEngine:c,canvas:d,disallowCanvasResizing:e});return n?k:k.data}finally{b||f._postWorkerMessage(c,"close")}}setGrayscaleWeights(a,b,c,d=!0){f._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:b,blue:c,useIntegerApproximation:d})}setInversionMode(a){f._postWorkerMessage(this._qrEnginePromise,"inversionMode",
