...
typeof d?d:this._calculateScanRegion);this._preferredCamera=b.preferredCamera||f||this._preferredCamera;this._legacyCanvasSize="number"===typeof c?c:"number"===typeof d?d:this._legacyCanvasSize;this._onPlay=this._onPlay.bind(this);this._onLoadedMetaData=this._onLoadedMetaData.bind(this);this._onVisibilityChange=this._onVisibilityChange.bind(this);this._updateOverlay=this._updateOverlay.bind(this);a.disablePictureInPicture=!0;a.playsInline=!0;a.muted=!0;let h=!1;a.hidden&&(a.hidden=!1,h=!0);document.body.contains(a)||
(document.body.appendChild(a),h=!0);c=a.parentElement;if(b.highlightScanRegion||b.highlightCodeOutline)d=!!b.overlay,this.$overlay=b.overlay||document.createElement("div"),f=this.$overlay.style,f.position="absolute",f.display="none",f.pointerEvents="none",this.$overlay.classList.add("scan-region-highlight"),!d&&b.highlightScanRegion&&(f.outline="#e9b213 solid 5px",c.insertBefore(this.$overlay,this.$video.nextSibling)),b.highlightCodeOutline&&(this.$codeOutlineHighlight=document.createElementNS("http://www.w3.org/2000/svg",
"svg"),b=document.createElementNS("http://www.w3.org/2000/svg","polygon"),this.$codeOutlineHighlight.appendChild(b),this.$overlay.appendChild(this.$codeOutlineHighlight),b=this.$codeOutlineHighlight.style,b.width="100%",b.height="100%",this.$codeOutlineHighlight.setAttribute("preserveAspectRatio","none"),b.display="none",this.$codeOutlineHighlight.classList.add("code-outline-highlight"),b.fill="none",b.stroke="#e9b213",b.strokeWidth="4");this._scanRegion=this._calculateScanRegion(a);requestAnimationFrame(()=>
...
window.removeEventListener("resize",this._updateOverlay);this._destroyed=!0;this._flashOn=!1;this.stop();e._postWorkerMessage(this._qrEnginePromise,"close")}async start(){if(!(this._active&&!this._paused||this._destroyed||("https:"!==window.location.protocol&&console.warn("The camera stream is only accessible if the page is transferred via https."),this._active=!0,document.hidden)))if(this._paused=!1,this.$video.srcObject)await this.$video.play();else try{let {stream:a,facingMode:b}=await this._getCameraStream();
...
h=b.alsoTryWithoutScanRegion||!1,n=!0):b||c||d||f||h?console.warn("You're using a deprecated api for scanImage which will be removed in the future."):console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true.");b=!!c;try{let q,k;[c,q]=await Promise.all([c||e.createQrEngine(),e._loadImage(a)]);[d,k]=e._drawToCanvas(q,m,d,f);if(c instanceof Worker){let g=c;b||g.postMessage({type:"inversionMode",
data:"both"});return await new Promise((l,p)=>{let u,v,t;v=r=>{"qrResult"===r.data.type&&(g.removeEventListener("message",v),g.removeEventListener("error",t),clearTimeout(u),null!==r.data.data?l(n?{data:r.data.data,cornerPoints:e._convertPoints(r.data.cornerPoints,m)}:r.data.data):p(e.NO_QR_CODE_FOUND))};t=r=>{g.removeEventListener("message",v);g.removeEventListener("error",t);clearTimeout(u);p("Scanner error: "+(r?r.message||r:"Unknown Error"))};g.addEventListener("message",v);g.addEventListener("error",
t);u=setTimeout(()=>t("timeout"),1E4);let w=k.getImageData(0,0,d.width,d.height);g.postMessage({type:"decode",data:w},[w.data.buffer])})}return await Promise.race([new Promise((g,l)=>window.setTimeout(()=>l("Scanner error: timeout"),1E4)),(async()=>{try{let [g]=await c.detect(d);if(!g)throw e.NO_QR_CODE_FOUND;return n?{data:g.rawValue,cornerPoints:e._convertPoints(g.cornerPoints,m)}:g.rawValue}catch(g){throw`Scanner error: ${g.message||g}`;}})()])}catch(q){if(!m||!h)throw q;let k=await e.scanImage(a,
{qrEngine:c,canvas:d,disallowCanvasResizing:f});return n?k:k.data}finally{b||e._postWorkerMessage(c,"close")}}setGrayscaleWeights(a,b,c,d=!0){e._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:b,blue:c,useIntegerApproximation:d})}setInversionMode(a){e._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}static async createQrEngine(a=e.WORKER_PATH){return"BarcodeDetector"in window&&BarcodeDetector.getSupportedFormats&&(await BarcodeDetector.getSupportedFormats()).includes("qr_code")?
...
a.cornerPoints.map(({x:b,y:c})=>`${b},${c}`).join(" ")),this.$codeOutlineHighlight.style.display="")):this.$codeOutlineHighlight&&!this._codeOutlineHighlightRemovalTimeout&&(this._codeOutlineHighlightRemovalTimeout=setTimeout(()=>this.$codeOutlineHighlight.style.display="none",100))}this._scanFrame()})}_onDecodeError(a){a!==e.NO_QR_CODE_FOUND&&console.log(a)}async _getCameraStream(){if(!navigator.mediaDevices)throw"Camera not found.";let a=/^(environment|user)$/.test(this._preferredCamera)?"facingMode":
...
c("Image load error"):b()};a.addEventListener("load",d);a.addEventListener("error",d)})}static async _postWorkerMessage(a,b,c){a=await a;a instanceof Worker&&a.postMessage({type:b,data:c})}}e.DEFAULT_CANVAS_SIZE=400;e.NO_QR_CODE_FOUND="No QR code found";e.WORKER_PATH="qr-scanner-worker.min.js";return e})
