...
console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true.");let t=!!d;try{let m,r;[d,m]=yield Promise.all([d||b.createQrEngine(),b._loadImage(a)]);[f,r]=b._drawToCanvas(m,l,f,g);let n;if(d instanceof Worker){let h=d;t||b._postWorkerMessageSync(h,"inversionMode","both");n=yield new Promise((q,z)=>{let x,v,w,y=-1;v=u=>{u.data.id===y&&(h.removeEventListener("message",v),h.removeEventListener("error",
w),clearTimeout(x),null!==u.data.data?q({data:u.data.data,cornerPoints:b._convertPoints(u.data.cornerPoints,l),bytes:u.data.bytes}):z(b.NO_QR_CODE_FOUND))};w=u=>{h.removeEventListener("message",v);h.removeEventListener("error",w);clearTimeout(x);z("Scanner error: "+(u?u.message||u:"Unknown Error"))};h.addEventListener("message",v);h.addEventListener("error",w);x=setTimeout(()=>w("timeout"),1E4);let A=r.getImageData(0,0,f.width,f.height);y=b._postWorkerMessageSync(h,"decode",A,[A.data.buffer])})}else n=
yield Promise.race([new Promise((h,q)=>window.setTimeout(()=>q("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var [h]=yield d.detect(f);if(!h)throw b.NO_QR_CODE_FOUND;return{data:h.rawValue,cornerPoints:b._convertPoints(h.cornerPoints,l),bytes:h.rawValue.split("").map(q=>q.charCodeAt(0))}}catch(q){h=q.message||q;if(/not implemented|service unavailable/.test(h))return b._disableBarcodeDetector=!0,b.scanImage(a,{scanRegion:l,canvas:f,disallowCanvasResizing:g,
alsoTryWithoutScanRegion:k});throw`Scanner error: ${h}`;}}))()]);return p?n:n.data}catch(m){if(!l||!k)throw m;let r=yield b.scanImage(a,{qrEngine:d,canvas:f,disallowCanvasResizing:g});return p?r:r.data}finally{t||b._postWorkerMessage(d,"close")}})}setGrayscaleWeights(a,c,d,f=!0){b._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:c,blue:d,useIntegerApproximation:f})}setInversionMode(a){b._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}static createQrEngine(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a&&
...
this.$video,c=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,k=a.offsetLeft,l=a.offsetTop,p=window.getComputedStyle(a),t=p.objectFit,m=c/d,r=f/g;switch(t){case "none":var n=c;var h=d;break;case "fill":n=f;h=g;break;default:("cover"===t?m>r:m<r)?(h=g,n=h*m):(n=f,h=n/m),"scale-down"===t&&(n=Math.min(n,c),h=Math.min(h,d))}var [q,z]=p.objectPosition.split(" ").map((v,w)=>{const y=parseFloat(v);return v.endsWith("%")?(w?g-h:f-n)*y/100:y});p=this._scanRegion.width||c;r=this._scanRegion.height||
d;t=this._scanRegion.x||0;var x=this._scanRegion.y||0;m=this.$overlay.style;m.width=`${p/c*n}px`;m.height=`${r/d*h}px`;m.top=`${l+z+x/d*h}px`;d=/scaleX\(-1\)/.test(a.style.transform);m.left=`${k+(d?f-q-n:q)+(d?c-t-p:t)/c*n}px`;m.transform=a.style.transform}})}static _convertPoints(a,c){if(!c)return a;let d=c.x||0,f=c.y||0,g=c.width&&c.downScaledWidth?c.width/c.downScaledWidth:1;c=c.height&&c.downScaledHeight?c.height/c.downScaledHeight:1;for(let k of a)k.x=k.x*g+d,k.y=k.y*c+f;return a}_scanFrame(){!this._active||
