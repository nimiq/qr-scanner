...
console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true.");let r=!!d;try{let m,q;[d,m]=yield Promise.all([d||c.createQrEngine(),c._loadImage(a)]);[f,q]=c._drawToCanvas(m,l,f,g);let n;if(d instanceof Worker){let h=d;r||h.postMessage({type:"inversionMode",data:"both"});n=yield new Promise((t,z)=>{let x,v,w;v=u=>{"qrResult"===u.data.type&&(h.removeEventListener("message",v),h.removeEventListener("error",
w),clearTimeout(x),null!==u.data.data?t({data:u.data.data,cornerPoints:c._convertPoints(u.data.cornerPoints,l)}):z(c.NO_QR_CODE_FOUND))};w=u=>{h.removeEventListener("message",v);h.removeEventListener("error",w);clearTimeout(x);z("Scanner error: "+(u?u.message||u:"Unknown Error"))};h.addEventListener("message",v);h.addEventListener("error",w);x=setTimeout(()=>w("timeout"),1E4);let y=q.getImageData(0,0,f.width,f.height);h.postMessage({type:"decode",data:y},[y.data.buffer])})}else n=yield Promise.race([new Promise((h,
