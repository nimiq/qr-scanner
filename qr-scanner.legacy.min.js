...
c.polyfillIsolated=function(a,b,d){var e=a.split(".");a=1===e.length;var f=e[0];f=!a&&f in c.polyfills?c.polyfills:c.global;for(var g=0;g<e.length-1;g++){var k=e[g];if(!(k in f))return;f=f[k]}e=e[e.length-1];d=c.IS_SYMBOL_NATIVE&&"es6"===d?f[e]:null;b=b(d);null!=b&&(a?c.defineProperty(c.polyfills,e,{configurable:!0,writable:!0,value:b}):b!==d&&(void 0===c.propertyToPolyfillSymbol[e]&&(a=1E9*Math.random()>>>0,c.propertyToPolyfillSymbol[e]=c.IS_SYMBOL_NATIVE?c.global.Symbol(e):c.POLYFILL_PREFIX+a+"$"+
...
c.asyncExecutePromiseGenerator=function(a){function b(e){return a.next(e)}function d(e){return a.throw(e)}return new Promise(function(e,f){function g(k){k.done?e(k.value):Promise.resolve(k.value).then(b,d).then(g,f)}g(a.next())})};c.asyncExecutePromiseGeneratorFunction=function(a){return c.asyncExecutePromiseGenerator(a())};c.asyncExecutePromiseGeneratorProgram=function(a){return c.asyncExecutePromiseGenerator(new c.generator.Generator_(new c.generator.Engine_(a)))};
...
this.$scanRegionHighlight.style.position="absolute",this.$scanRegionHighlight.style.display="none",this.$scanRegionHighlight.style.outline="rgba(255, 255, 255, .3) solid 7px");this._scanRegion=this._calculateScanRegion(a);this._onPlay=this._onPlay.bind(this);this._onLoadedMetaData=this._onLoadedMetaData.bind(this);this._onVisibilityChange=this._onVisibilityChange.bind(this);a.disablePictureInPicture=!0;a.playsInline=!0;a.muted=!0;let g=!1;a.hidden&&(a.hidden=!1,g=!0);document.body.contains(a)||(document.body.appendChild(a),
g=!0);let k=a.parentElement;requestAnimationFrame(()=>{let m=window.getComputedStyle(a),p=window.getComputedStyle(k);"none"===m.display&&(a.style.setProperty("display","block","important"),g=!0);"visible"!==m.visibility&&(a.style.setProperty("visibility","visible","important"),g=!0);g&&(console.warn("QrScanner has overwritten the video hiding style to avoid Safari stopping the playback."),a.style.opacity="0",a.style.width="0",a.style.height="0",delete this.$scanRegionHighlight);if(this.$scanRegionHighlight){["relative",
"absolute","fixed","sticky"].includes(p.position)||(console.warn("QrScanner has overwritten the video parent's position for correct display of the scan region highlight. To avoid this, set the position to relative, absolute, fixed or sticky."),k.style.setProperty("position","absolute","important"));if(p.width!==m.width||p.height!==m.height)console.warn("QrScanner has overwritten the video parent's width and height for correct display of the scan region highlight. To avoid this, make the parent as big as the video or the video as big as the parent."),
k.style.setProperty("width","max-content","important"),k.style.setProperty("height","max-content","important");this._updateScanRegionHighlight();k.appendChild(this.$scanRegionHighlight)}});a.addEventListener("play",this._onPlay);a.addEventListener("loadedmetadata",this._onLoadedMetaData);document.addEventListener("visibilitychange",this._onVisibilityChange);this._qrEnginePromise=h.createQrEngine()}static hasCamera(){return c.asyncExecutePromiseGeneratorFunction(function*(){try{return!!(yield h.listCameras(!1)).length}catch(a){return!1}})}static listCameras(a=
...
b._preferredCamera&&(b._preferredCamera=a,yield b._restartVideoStream())})}static scanImage(a,b,d,e,f=!1,g=!1){return c.asyncExecutePromiseGeneratorFunction(function*(){let k,m=!1;b&&("scanRegion"in b||"qrEngine"in b||"canvas"in b||"disallowCanvasResizing"in b||"alsoTryWithoutScanRegion"in b||"returnDetailedScanResult"in b)?(k=b.scanRegion,d=b.qrEngine,e=b.canvas,f=b.disallowCanvasResizing||!1,g=b.alsoTryWithoutScanRegion||!1,m=!0):b||d||e||f||g?console.warn("You're using a deprecated api for scanImage which will be removed in the future."):
console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true.");let p=!!d;try{let t,q;[d,t]=yield Promise.all([d||h.createQrEngine(),h._loadImage(a)]);[e,q]=h._drawToCanvas(t,k,e,f);if(d instanceof Worker){let l=d;p||l.postMessage({type:"inversionMode",data:"both"});return yield new Promise((v,x)=>{let w,u,r;u=n=>{"qrResult"===n.data.type&&(l.removeEventListener("message",u),l.removeEventListener("error",
r),clearTimeout(w),null!==n.data.data?v(m?{data:n.data.data,cornerPoints:h._convertPoints(n.data.cornerPoints,k)}:n.data.data):x(h.NO_QR_CODE_FOUND))};r=n=>{l.removeEventListener("message",u);l.removeEventListener("error",r);clearTimeout(w);x("Scanner error: "+(n?n.message||n:"Unknown Error"))};l.addEventListener("message",u);l.addEventListener("error",r);w=setTimeout(()=>r("timeout"),1E4);let y=q.getImageData(0,0,e.width,e.height);l.postMessage({type:"decode",data:y},[y.data.buffer])})}return yield Promise.race([new Promise((l,
v)=>window.setTimeout(()=>v("Scanner error: timeout"),1E4)),(()=>c.asyncExecutePromiseGeneratorFunction(function*(){try{let [l]=yield d.detect(e);if(!l)throw h.NO_QR_CODE_FOUND;return m?{data:l.rawValue,cornerPoints:h._convertPoints(l.cornerPoints,k)}:l.rawValue}catch(l){throw`Scanner error: ${l.message||l}`;}}))()])}catch(t){if(!k||!g)throw t;let q=yield h.scanImage(a,{qrEngine:d,canvas:e,disallowCanvasResizing:f});return m?q:q.data}finally{p||h._postWorkerMessage(d,"close")}})}setGrayscaleWeights(a,
...
b)/2),width:b,height:b,downScaledWidth:this._legacyCanvasSize,downScaledHeight:this._legacyCanvasSize}}_updateScanRegionHighlight(){if(this.$scanRegionHighlight){var a=this.$video,b=a.videoWidth,d=a.videoHeight;this.$scanRegionHighlight.style.width=`${(this._scanRegion.width||b)/b*100}%`;this.$scanRegionHighlight.style.height=`${(this._scanRegion.height||d)/d*100}%`;this.$scanRegionHighlight.style.top=`${(this._scanRegion.y||0)/d*100}%`;a=/scaleX\(-1\)/.test(a.style.transform);this.$scanRegionHighlight.style[a?
"right":"left"]=`${(this._scanRegion.x||0)/b*100}%`;this.$scanRegionHighlight.style[a?"left":"right"]="unset"}}static _convertPoints(a,b){if(!b)return a;let d=b.x||0,e=b.y||0,f=b.width&&b.downScaledWidth?b.width/b.downScaledWidth:1;b=b.height&&b.downScaledHeight?b.height/b.downScaledHeight:1;for(let g of a)g.x=g.x*f+d,g.y=g.y*b+e;return a}_scanFrame(){!this._active||this.$video.paused||this.$video.ended||requestAnimationFrame(()=>{const a=this;return c.asyncExecutePromiseGeneratorFunction(function*(){if(!(1>=
a.$video.readyState)){try{var b=yield h.scanImage(a.$video,{scanRegion:a._scanRegion,qrEngine:a._qrEnginePromise,canvas:a.$canvas})}catch(d){if(!a._active)return;(d.message||d).includes("service unavailable")&&(a._qrEnginePromise=h.createQrEngine());a._onDecodeError(d)}b&&a._onDecode?a._onDecode(b):b&&a._legacyOnDecode&&a._legacyOnDecode(b.data)}a._scanFrame()})})}_onDecodeError(a){a!==h.NO_QR_CODE_FOUND&&console.log(a)}_getCameraStream(){const a=this;return c.asyncExecutePromiseGeneratorFunction(function*(){if(!navigator.mediaDevices)throw"Camera not found.";
let b=/^(environment|user)$/.test(a._preferredCamera)?"facingMode":"deviceId",d=[{width:{min:1024}},{width:{min:768}},{}],e=d.map(f=>Object.assign({},f,{[b]:{exact:a._preferredCamera}}));for(let f of[...e,...d])try{let g=yield navigator.mediaDevices.getUserMedia({video:f,audio:!1}),k=a._getFacingMode(g)||(f.facingMode?a._preferredCamera:"environment"===a._preferredCamera?"user":"environment");return{stream:g,facingMode:k}}catch(g){}throw"Camera not found.";})}_restartVideoStream(){const a=this;return c.asyncExecutePromiseGeneratorFunction(function*(){let b=
...
a.videoWidth||a.width,m=b&&b.height?b.height:a.videoHeight||a.height;e||(e=b&&b.downScaledWidth?b.downScaledWidth:k,b=b&&b.downScaledHeight?b.downScaledHeight:m,d.width!==e&&(d.width=e),d.height!==b&&(d.height=b));b=d.getContext("2d",{alpha:!1});b.imageSmoothingEnabled=!1;b.drawImage(a,f,g,k,m,0,0,d.width,d.height);return[d,b]}static _loadImage(a){return c.asyncExecutePromiseGeneratorFunction(function*(){if(a instanceof Image)return yield h._awaitImageLoad(a),a;if(a instanceof HTMLVideoElement||a instanceof
